...
<script>
    ...
    $(document).ready(function() {
        ...
        var seatClass = '<%= passenger.getSeatClass() != null ? passenger.getSeatClass() : "" %>';
        var seatPosition = '<%= passenger.getSeatPosition() != null ? passenger.getSeatPosition() : "" %>';

        console.log("Loading data for passenger:", pNum, "Date:", dptDate, "Time:", dptTime, "Class:", seatClass, "Position:", seatPosition);

        // 도착 시간 설정
        $('#arrTime_<%= passenger.getPNum() %>').val('<%= new java.text.SimpleDateFormat("HH:mm").format(passenger.getArrTime()) %>');

        // 출발 시간 목록 가져오기
        getDepartureTimes(dptDate, pNum, fltCode, dptTime);

        // 좌석 위치 목록 가져오기 (좌석 등급이 있는 경우에만)
        if (seatClass && seatClass !== 'null' && seatClass !== '') {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data && data.length > 0) {
                    ...
                    // 시간 데이터가 없는 경우 기본 시간 설정
                    timeSelect.append('<option value="00:00" selected>00:00</option>');
                }
            },
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data && data.arrDate && data.arrTime) {
                    ...
                    $('#arrDate_' + pNum).val(formattedDate);
                    $('#arrTime_' + pNum).val('00:00');
                }
            },
            error: function(xhr, status, error) {
    ...
    function getAvailableSeatPositions(seatClass, pNum, fltCode, selectedPosition) {
        if (!seatClass || seatClass === 'null') return;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (selectedPosition && selectedPosition !== 'null' && selectedPosition !== 'undefined') {
                ...
                if (data && data.length > 0) {
                    ...
                    // 선택된 위치가 없으면 첫 번째 위치 선택
                    if (!selectedPosition && data.length > 0) {
                        positionSelect.val(data[0].value);
                    }
                } else {
                    // 좌석 위치 데이터가 없는 경우 기본 위치 설정
                    positionSelect.append('<option value="1A" selected>1A</option>');
                }
            },
            error: function(xhr, status, error) {
